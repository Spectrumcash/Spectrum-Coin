...
int nAnonymizeSpectrumAmount = 500;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Spectrum
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Spectrum
    // Mac: ~/Library/Application Support/Spectrum
    // Unix: ~/.Spectrum
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Spectrum";
#else
    fs::path pathRet;
...
void createConf()       //Automatic Spectrum.conf generation
{
    srand(time(NULL));
    ...
    const char* nodes =  "\nrpcport=44821"                  //List of known nodes, to be periodically updated
                         "\nrpcallowip=127.0.0.1"
                         "\ndaemon=1"
                         "\nserver=1"
                         "\nlistenonion=0"
                         "\naddnode=149.28.57.142"
                         "\naddnode=95.179.142.200"
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Spectrum.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) //Spectrum.conf doesn't exist
    {
        createConf();
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Spectrumd.pid"));
