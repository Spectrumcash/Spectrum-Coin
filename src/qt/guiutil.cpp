...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Spectrum"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("Spectrum://"))
    {
        uri.replace(0, 12, "Spectrum:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Spectrum.lnk";
}
...
{
    return GetAutostartDir() / "Spectrum.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Spectrum-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Spectrum-qt [" + tr("command-line options") + "]                     " + "\n";
