...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSpectrumAmount"))
        settings.setValue("nAnonymizeSpectrumAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeSpectrumAmount = settings.value("nAnonymizeSpectrumAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeSpectrumAmount"))
        SoftSetArg("-anonymizeSpectrumamount", settings.value("nAnonymizeSpectrumAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeSpectrumAmount:
            nAnonymizeSpectrumAmount = value.toInt();
            settings.setValue("nAnonymizeSpectrumAmount", nAnonymizeSpectrumAmount);
            emit anonymizeSpectrumAmountChanged(nAnonymizeSpectrumAmount);
            break;
        default:
            break;
        }
