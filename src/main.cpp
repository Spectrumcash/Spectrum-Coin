...
const string strMessageMagic = "Spectrum Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Rather not work on nonstandard transactions (unless -testnet)
    //alot of Spectrum transactions seem non standard, its a bug so we have to accept these, the transactions have still been checekd to be valid and unspent.
    string reason;
...
int64_t GetProofOfWorkReward(int64_t nFees, unsigned int nHeight)
{
    ...
    if(pindexBest->nHeight < MASTERNODE_BLOCK)
    {
...
int64_t nSubsidy=0;
	...
	else if (nHeight >=300000 and nHeight <600000 )
	{
		nSubsidy = nCoinAge * COIN_YEAR_REWARD * 25 / (365 * 33 + 8);
	}
	else if (nHeight >=600000 and nHeight <900000 )
	{
		nSubsidy = nCoinAge * COIN_YEAR_REWARD * 20 / (365 * 33 + 8);
	}
	else if (nHeight >=900000 and nHeight <1200000 )
	{
		nSubsidy = nCoinAge * COIN_YEAR_REWARD * 15 / (365 * 33 + 8);
	}
	else if (nHeight >=1200000 and nHeight <1500000 )
	{
		nSubsidy = nCoinAge * COIN_YEAR_REWARD * 10 / (365 * 33 + 8);
	}
	else if (nHeight >=1500000 and nHeight <1800000 )
	{
		nSubsidy = nCoinAge * COIN_YEAR_REWARD * 8 / (365 * 33 + 8);
	}
	else
	{
...
#ifdef ENABLE_WALLET
// Spectrum: attempt to generate suitable proof-of-stake
...
{
    int64_t ret = static_cast<int64_t>(blockValue * 0.37); //67%
